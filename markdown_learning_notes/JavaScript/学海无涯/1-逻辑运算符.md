...
虽然被称为“逻辑”，这些运算符却可以被应用于**任意类型的值**，而不仅仅是**布尔值**。结果也同样可能是**任意类型**。让我们来详细看一下。
...
如果 `currentUser` 和 `defaultUser` 都是假值，那么结果就是 `"unnamed"`。 2. **短路取值。**
...
要知道：“与运算 `&&` 在或操作符 `||` 之前执行” 与运算 `&&` 的优先级比或运算 `||` 要高，所以它将会比或运算先执行。
